# --- START OF MODIFIED transcription_tab_ui.py ---
...
# from gui import ModernTranscriptionApp # Avoid direct import if possible
...
class TranscriptionTabUI:
    ...
    def __init__(self, parent_notebook, gui_app):
        ...
        # Store reference to main app for accessing shared methods/vars
        # Be mindful of dependencies - try to keep this component self-contained
        self.gui_app = gui_app
        self.transcriber = gui_app.transcriber # Need transcriber ref
    ...
    def _create_widgets(self):
        ...
        self.browse_button = ttk.Button(self.file_frame, text="", command=self.select_file, style="Action.TButton") # Command calls local method now
        ...
        self.model_combobox.set(self.model_var.get())
        ...
        self.model_combobox.bind("<<ComboboxSelected>>", self.update_model_description) # Command calls local method now
        ...
        self.language_combobox.set(self.transcription_language_var.get())
        ...
        # Tooltip binding needs the local methods now
        self.gpu_check.bind("<Enter>", self.show_gpu_tooltip)
        self.gpu_check.bind("<Leave>", self._on_leave_tooltip) # Bind leave here

        # Buttons
        buttons_frame = ttk.Frame(self.frame)
        buttons_frame.grid(row=2, column=0, sticky="ew", pady=(0, 15))
        self.start_button = ttk.Button(buttons_frame, text="", command=self.start_transcription, style="Primary.TButton") # Command calls local method
        self.start_button.pack(side=tk.LEFT, padx=5, pady=5)
        self.stop_button = ttk.Button(buttons_frame, text="", command=self.stop_transcription, style="Action.TButton", state=tk.DISABLED) # Command calls local method
        self.stop_button.pack(side=tk.LEFT, padx=5, pady=5)

        action_buttons_right = ttk.Frame(buttons_frame)
        action_buttons_right.pack(side=tk.RIGHT)
        self.save_button = ttk.Button(action_buttons_right, text="", command=self.save_transcription, style="Action.TButton") # Command calls local method
        self.save_button.pack(side=tk.RIGHT, padx=5, pady=5)
        self.copy_button = ttk.Button(action_buttons_right, text="", command=self.copy_to_clipboard, style="Action.TButton") # Command calls local method
        ...
        self.results_notebook.add(transcription_result_frame, text="")
        ...
        self.console_output = scrolledtext.ScrolledText(console_text_frame, wrap=tk.WORD, width=80, height=10, background="#f0f0f0", foreground="#333", font=("Consolas", 10), borderwidth=0, relief="flat", state=tk.DISABLED) # Start disabled
        self.console_output.grid(row=0, column=0, sticky="nsew", padx=1, pady=1)


    # --- Methods Moved from gui.py (now local to this tab) ---
    ...
    def update_ui_text(self):
        ...
        # Update model description (might also be called by main app's language change)
        self.update_model_description()


    def update_model_description(self, event=None):
        try:
            if hasattr(self, 'model_desc_label') and self.model_desc_label.winfo_exists():
                 selected_model = self.model_var.get()
                 # Access descriptions dict via gui_app
                 desc_key = self.gui_app.model_descriptions.get(selected_model, "model_desc_large")
                 self.model_desc_var.set(self.gui_app.translate(desc_key))
    ...
    def select_file(self):
        ...
        file = filedialog.askopenfilename(
            ...
            parent=self.frame # Set parent to tab frame for modality
        )
        if file:
            self.file_path_var.set(file)
            filename = os.path.basename(file)
            # Attempt to get info for WAV using transcriber via gui_app
            if file.lower().endswith(".wav"):
                try:
                    # Use the transcriber's helper method via gui_app reference
                    file_info = self.transcriber.get_audio_info(file)
                    if file_info:
                        duration, channels, rate = file_info
                        from utils import format_duration # Local import or move to top
                        ...
                        self.console_output_insert(info_msg) # Use local method
                    else:
    ...
    def start_transcription(self):
        ...
        language_code = self.gui_app.get_language_code(self.transcription_language_var.get()) # Use main app helper
        ...
        # Call transcriber via the reference
        self.transcriber.start_transcription_async(
            input_file, model_type, language_code, use_gpu, self.gui_app.system_type
        )
    ...
    def stop_transcription(self):
         if hasattr(self.transcriber, 'request_stop'):
             ...
             # Update status variables via gui_app
    ...
    def copy_to_clipboard(self):
        ...
        if not text_to_copy:
            ...
            return
        try:
            # Use the frame's clipboard methods
    ...
    def save_transcription(self):
        ...
        file_path = filedialog.asksaveasfilename(
            title=self.gui_app.translate("save_button"), defaultextension=".txt",
            filetypes=[("Text files", "*.txt"), ("All files", "*.*")],
            initialfile=suggested_filename, initialdir=initial_dir, parent=self.frame
        )
        if file_path:
            try:
    ...
    def show_gpu_tooltip(self, event):
        self._destroy_tooltip() # Destroy existing first
        try:
            self.gpu_tooltip = tk.Toplevel(self.frame) # Parent to tab frame
            self.gpu_tooltip.wm_overrideredirect(True)
            x, y = self.frame.winfo_pointerx() + 15, self.frame.winfo_pointery() + 10
            ...
            # Leave event is bound during widget creation
        except Exception as e:
            print(f"Error showing GPU tooltip: {e}")
            self._destroy_tooltip() # Clean up if creation failed

    def _destroy_tooltip(self):
        if hasattr(self, '_tooltip_after_id') and self._tooltip_after_id:
            try: self.frame.after_cancel(self._tooltip_after_id)
        ...
        # No need to unbind here if bound only once in _create_widgets

    def _on_leave_tooltip(self, event=None):
        self._destroy_tooltip()

    # --- Methods for UI Updates from Backend ---

    def update_progress_state(self, task_text, status_text, progress_mode):
        """Updates the progress bar and status labels specific to this tab."""
        try:
            if not self.frame.winfo_exists(): return
            # Update shared status vars via gui_app is handled by caller
            # Update local widgets
            self.current_task_var.set(task_text) # Update label text
    ...
    def finalize_ui_state(self, success, interrupted):
        """Finalizes button states and progress bar for this tab."""
        try:
            if not self.frame.winfo_exists(): return
            # Update buttons
            ...
            if hasattr(self, 'progress_bar') and self.progress_bar.winfo_exists():
                ...
                self.progress_var.set(0)
            # Status/Task variable updates are handled by the main app caller
    ...
    # --- Methods for Direct Text Manipulation ---
    ...
    def result_text_set(self, text):
        try:
            if hasattr(self, 'result_text') and self.result_text.winfo_exists():
                 ...
                 # Keep state NORMAL? Or set back to DISABLED if needed?
...
# --- END OF MODIFIED transcription_tab_ui.py ---
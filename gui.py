...
import os
import sys
import typing # Import typing for hints
...
# from utils import format_duration # Not needed directly here anymore
...
class ModernTranscriptionApp:
    ...
    def __init__(self, root):
        ...
        self.update_model_description() # Call after model_var is potentially set by config
    ...
    def _gather_current_config(self) -> dict:
        settings = {
            ...
            "llm_provider": None, "llm_model": None, "llm_api_key": ""
        }
        if hasattr(self, 'llm_tab') and self.llm_tab:
            settings["llm_provider"] = self.llm_tab.llm_provider_var.get()
            settings["llm_model"] = self.llm_tab.llm_model_var.get()
            settings["llm_api_key"] = self.llm_tab.llm_api_key_var.get()
        return settings
         ...
         if hasattr(self, 'llm_tab') and self.llm_tab:
             ...
             if hasattr(self, 'loaded_llm_provider') and self.loaded_llm_provider:
                 ...
                 if hasattr(self, 'loaded_llm_model') and self.loaded_llm_model:
                     self.root.after(50, lambda: self.llm_tab.set_model_safely(self.loaded_llm_model))
    ...
    # **** FIX: Add the on_language_select method back ****
    def on_language_select(self, event=None):
        """Callback when a language is selected in the header combobox."""
        # The trace on self.current_language handles the actual update logic.
        # This method simply needs to exist for the binding.
        pass
    ...
    def change_language(self, *args):
        ...
        self.update_model_description() # Update model description specifically
        print(f"UI updated for language: {self.current_language.get()}")

    # **** FIX: Add the get_language_code method back ****
    def get_language_code(self, language_name: str) -> str:
        ...
        return language_map.get(language_name.lower(), "italian") # Default to italian


    def update_model_description(self, event=None):
        """Updates the model description label in the transcription tab."""
        if hasattr(self, 'transcription_tab'):
            self.root.after_idle(self.transcription_tab.update_model_description)

    def update_transcription_path_callback(self, file_path):
         if file_path and os.path.exists(file_path):
             self.file_path.set(file_path)
             self._print(f"Audio file path set from recorder: {os.path.basename(file_path)}\n")
             if hasattr(self, 'main_notebook') and hasattr(self, 'transcription_tab'):
                  ...
                  except tk.TclError: print("Could not switch to transcription tab (TclError).", file=sys.__stderr__)
                  except Exception as e: print(f"Could not switch to transcription tab (Error: {e}).", file=sys.__stderr__)
         else: self._print(f"Invalid file path received from recorder: {file_path}\n")

    def _print(self, message):
        if hasattr(self, 'transcription_tab'): self.transcription_tab.console_output_insert(message)
        else: print(f"Fallback print (transcription_tab not ready): {message.strip()}", file=sys.__stderr__)
    ...
    def _show_error(self, error_key, **kwargs):
         title = self.translate("error_title"); message = self.translate(error_key).format(**kwargs)
         self.root.after_idle(lambda: messagebox.showerror(title, message, parent=self.root))

    def _show_info(self, info_key, **kwargs):
         title = self.translate("info_title"); message = self.translate(info_key).format(**kwargs)
